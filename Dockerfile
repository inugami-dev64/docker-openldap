...
WORKDIR /app/openldap
RUN wget https://openldap.org/software/download/OpenLDAP/openldap-release/openldap-${VERSION}.tgz
RUN wget https://openldap.org/software/download/OpenLDAP/openldap-release/openldap-${VERSION}.tgz.asc

RUN cat <<EOF | gpg --import
...
RUN gpg --verify openldap-${VERSION}.tgz.asc
RUN tar -xvf openldap-${VERSION}.tgz

# Build OpenLDAP
WORKDIR /app/openldap/openldap-${VERSION}
RUN ./configure --with-pic=no --enable-shared=no --prefix=/usr/local
